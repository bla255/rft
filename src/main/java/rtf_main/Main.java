...
public class Main {
    ...
    public static MoziJegyrendszer run() {
        ...
        Film masodikfilm = new Film("2", "Ricsi filmje", "Ricsi osszecsap a focihuliganokkal a varosba", "Drama", 90, "2022-01-01", "0");
        Film harmadikfilm = new Film("3", "Bela es a gombgyar", "Bela a gyarban nem dolgozik rendesen", "Eletrajz", 5, "2000-01-01", "0");
        ...
        //Teszt: rafoglalok egy mar foglalt szekre
        System.out.println("\n\n-----------------TESZT - RAFOGLALAS ------------------");
        List<Szek> kivalasztottSzek1 = Arrays.asList(vetites1.getSzekek().get("1-3"));
        Foglalas foglalas1 = moziJegyrendszer.foglaltJegyek(felhasznalo, vetites1, kivalasztottSzek1);
        if (foglalas1 != null) {
            System.out.println("Szekek befoglalasa sikeres. Tranzakcioazonosito: " + foglalas.getId());
            moziJegyrendszer.FoglalasVeglegesit(foglalas.getId());

        } else {
            System.out.println("Szekek befoglalasa nem sikerult, a hely foglalt");
        }
